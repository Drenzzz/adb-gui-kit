...
export function AppManagerQuickControlsCard({
  quickControlSelects,
  selectedCount,
  isBusy,
  onEnableSelected,
      ...
      <CardContent className="space-y-5">
                ...
                <DropdownMenuContent align="start" className="w-48">
                  {config.options.map((option) => (
                    <DropdownMenuItem
                      ...
                      className={cn(
                        option.value === config.activeValue &&
        ...
        <div className="rounded-2xl border border-dashed border-muted-foreground/40 bg-muted/30 p-4">
          ...
          <p className="text-xs text-muted-foreground">
            {selectedCount === 0
              ? "Pick packages from the table to enable bulk actions."
              : "Use the buttons below to apply actions to the selected packages."}
          </p>
        </div>
