...
import { ArrowDownAZ, ArrowUp, ArrowUpAZ, Loader2, MoreHorizontal, RefreshCw, Search } from "lucide-react";
      ...
      <CardContent className="space-y-6 p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Browse device storage, move files, and manage folders without leaving this screen.</p>
          </div>
          <div className="grid w-full grid-cols-2 gap-3 sm:grid-cols-4 md:w-auto">
            {[
              ...
              <div key={metric.label} className="rounded-xl border border-border/70 bg-muted/30 px-4 py-3 text-center md:text-left">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex flex-1 flex-wrap items-center gap-2">
            <div className="inline-flex rounded-lg border border-border/60 bg-muted/30 p-1">
              <Button variant="ghost" size="icon" onClick={onBack} disabled={!canGoBack || isBusy} aria-label="Go up">
                <ArrowUp className="h-4 w-4" />
              </Button>
              <Button variant="ghost" size="icon" onClick={onRefresh} disabled={isBusy} aria-label="Refresh">
          ...
          <div className="flex w-full flex-col gap-2 sm:flex-row sm:items-center sm:justify-end">
            <div className="relative w-full sm:w-64">
              ...
              <Input placeholder="Search files or folders..." className="pl-9" value={searchTerm} onChange={(e) => onSearchTermChange(e.target.value)} />
            </div>
            <div className="flex items-center gap-2">
              <select
                value={sortField}
                onChange={(e) => onSortFieldChange(e.target.value as "name" | "date" | "size")}
              ...
              <Button variant="outline" size="icon" onClick={onToggleSortDirection} aria-label="Toggle sort direction">
        ...
        <div className="rounded-xl border border-border/60 bg-muted/20 px-4 py-3">
          <p className="text-sm text-muted-foreground">{selectedCount === 0 ? "Select items to enable rename, export, or delete actions." : `${selectedCount} item${selectedCount > 1 ? "s" : ""} selected.`}</p>
          <div className="mt-3 hidden flex-wrap gap-2 lg:flex">
            {actionItems.map((action) => (
              <Button key={action.key} variant={action.variant ?? "outline"} onClick={action.onClick} disabled={action.disabled}>
